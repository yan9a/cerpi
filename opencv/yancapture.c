...
IMG im,im2;
char filename[] = "pic0.bmp";
char filename2[] = "img0.bmp";
/* Bmp ---------- */
...
static int process_raw(unsigned char* po, unsigned char* pi, int size)
{
        int i,row,col;  
        ...
        */
        // change to 256 x 192 
        size = 256 * 192;
        for(i=0, row = 192, col = 0;i<size;i++, col++){
            if(col>=256){
                col = 0;
                row++;
            }
                po[i] = pi[row*1280+col];
        }
        return size;
}
...
{             
        ...
        for(j=0;j<im2.Width;j++){
                im2.Image[i][j]= im.Image[i*3][j*4];
        }
        SaveBMPFile(filename2,im2);
}
...
{
        im = AllocateImage(256,192);
        im2 = AllocateImage(64,64);
